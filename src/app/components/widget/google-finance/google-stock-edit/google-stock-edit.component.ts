          ...
          this.googleFinanceService.searchCompanyHistoricals(this.companyName, time, time)
            .subscribe((companyHistoricals) => {
              ...
              if (this.companyHistoricals === null || this.companyHistoricals.length === 0) {
                ...
                const timeY = yesterday.toLocaleString('en-US', {timeZone: 'America/New_York'}).slice(0, 10);
                this.googleFinanceService.searchCompanyHistoricals(this.companyName, timeY, timeY)
                  .subscribe((companyHistoricalsY) => {
                    this.companyHistoricals = companyHistoricalsY;
                    if (this.companyHistoricals === null || this.companyHistoricals.length === 0) {
                      alert('the stock not existed');
                    }else {
        ...
        this.googleFinanceService.searchCompanyHistoricals(companyName, time, time)
          .subscribe((companyHistoricals) => {
            ...
            if (this.companyHistoricals === null || this.companyHistoricals.length === 0) {
              ...
              yesterday.setDate(yesterday.getDate() - 1);
              const timeY = yesterday.toLocaleString('en-US', {timeZone: 'America/New_York'}).slice(0, 10);
              this.googleFinanceService.searchCompanyHistoricals(companyName, timeY, timeY)
                .subscribe((companyHistoricalsY) => {
                  this.companyHistoricals = companyHistoricalsY;
                  if (this.companyHistoricals === null || this.companyHistoricals.length === 0) {
                    alert('the stock not existed');
